...
from utils.response_parser import process_ai_response
    ...
    try:
        ...
        while True:
            ...
            message_content = get_message_content(
                file_path, included_file_content)
            ...
            try:
                ...
                if requested_files:
                    ...
                    if file_contents:
                        ...
                        logger.info(
                            f"Avatar (follow-up): {follow_up_response}")
                        write_to_file(os.path.join(
        ...
        # Cleanup: Wipe the CURRENT_RESPONSE_FILE
        try:
            with open(CURRENT_RESPONSE_FILE, 'w') as f:
                f.write('')  # Write an empty string to clear the file
            logger.info(f"Cleared {CURRENT_RESPONSE_FILE}")
        except Exception as e:
            logger.error(f"Error clearing {CURRENT_RESPONSE_FILE}: {e}")
